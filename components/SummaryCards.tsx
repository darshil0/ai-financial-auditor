...
const SummaryCards: React.FC<SummaryCardsProps> = ({ report }) => {
  ...
  const getColorClasses = (color: string) => {
    const map: Record<string, string> = {
      ...
      emerald:
        "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400 shadow-emerald-500/10",
      violet:
        "bg-violet-50 text-violet-600 dark:bg-violet-900/30 dark:text-violet-400 shadow-violet-500/10",
      amber:
        "bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400 shadow-amber-500/10",
    };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {cards.map((card, i) => (
        <div
          key={i}
          className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col justify-between transition-all hover:shadow-xl hover:-translate-y-1 relative overflow-hidden group"
        >
          ...
          <div className="flex justify-between items-start mb-6 relative z-10">
            ...
            {card.change !== null && (
              <div
                className={`flex items-center gap-0.5 text-xs font-black px-3 py-1.5 rounded-full shadow-sm ${
                  card.change >= 0
