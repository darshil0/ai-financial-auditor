...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const baseClasses =
    "w-64 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 h-screen z-[70] transition-transform duration-300 lg:translate-x-0";
  const mobileClasses = isOpen
    ? "translate-x-0 fixed inset-y-0 left-0"
    : "-translate-x-full fixed inset-y-0 left-0 lg:static";
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            {"badge" in item && item.badge > 0 && (
              <span className="ml-auto px-2 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded-full text-[10px] font-black animate-in zoom-in duration-300">
                {item.badge}
              </span>
            )}
          </button>
