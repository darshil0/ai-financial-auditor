...
import { X, CheckCircle2, AlertCircle, Loader2, Play, Activity, ShieldCheck, Bug } from 'lucide-react';
...
const DiagnosticsOverlay: React.FC<DiagnosticsOverlayProps> = ({ onClose }) => {
  const [tests, setTests] = useState<TestResult[]>([
    { id: '1', name: 'Responsive Layout Check', status: 'pending', message: 'Verifying container widths and stacking logic.' },
    { id: '2', name: 'Color Palette Contrast', status: 'pending', message: 'Checking WCAG 2.1 contrast ratios for legibility.' },
    { id: '3', name: 'Interactive States', status: 'pending', message: 'Testing hover, focus, and active button states.' },
    { id: '4', name: 'Asset Integrity', status: 'pending', message: 'Validating SVG and iconography rendering.' },
    { id: '5', name: 'LocalStorage Persistence', status: 'pending', message: 'Checking report vault write/read capability.' },
  ]);
        ...
        <div className="p-8 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Automation Suite v2.0</p>
            </div>
          </div>
          ...
          {tests.map(test => (
            ...
            >
              <div className={`p-2 rounded-xl flex-shrink-0 ${
                test.status === 'pass' ? 'text-emerald-500 bg-emerald-100 dark:bg-emerald-900/30' :
                test.status === 'running' ? 'text-blue-500 bg-blue-100 dark:bg-blue-900/30' :
              ...
              <div className="flex-1">
                <h4 className={`font-black text-sm mb-1 ${test.status === 'pass' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>
                  {test.name}
                </h4>
                <p className="text-xs text-slate-500 font-medium">{test.message}</p>
              </div>
              {test.status === 'pass' && (
                <span className="text-[10px] font-black uppercase text-emerald-600 bg-emerald-100 dark:bg-emerald-900/40 px-3 py-1 rounded-full">Passed</span>
              )}
            </div>
          ))}
        </div>
