...
const App: React.FC = () => {
  ...
  const refreshReports = () => {
    ...
    if (saved) {
      ...
      if (
        activeReportId &&
        !parsed.find((r: FinancialReport) => r.id === activeReportId)
      ) {
  ...
  return (
    <div
      className={`min-h-screen flex transition-colors duration-200 ${isDarkMode ? "bg-slate-900 text-slate-100" : "bg-slate-50 text-slate-900"}`}
    >
